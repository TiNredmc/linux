...
static int memlcdfb_init(struct memlcdfb_par *par){
	...
	ret = spi_write(par->spi, SendBuf, 2);// this will bring CS pin to low as normal state.
	mdelay(1);// delay 1ms
...
static void memlcdfb_update_display(struct memlcdfb_par *par){
	...
	u8 SendBuf[2] = {MLCD_UD, 0x00};
	u8 row, cpy, temp;

	par->ssbuf[0] = SendBuf[0];// Display update command
...
static int memlcdfb_probe(struct spi_device *spi){
	...
	vmem = (u8 *)kmalloc(MLCD_BUFSIZE, GFP_KERNEL);
